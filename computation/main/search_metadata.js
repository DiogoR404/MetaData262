...
function analysis(stmt, fileToAnalyse, results) {
    ...
    function mapper(stmt) {
        ...
        if (!results[fileToAnalyse].hasOwnProperty("syntactic_construct")) {
            results[fileToAnalyse]["syntactic_construct"] = [stmt.type];
        } else if (!results[fileToAnalyse]["syntactic_construct"].includes(stmt.type)) {
            ...
            case "Identifier":
                {
                    if (metadata_global.includes(stmt.name)) {
                        if (stmt.name === "let" || stmt.name === "const") {
                            ...
                            if (!results[fileToAnalyse].hasOwnProperty("builtIns")) {
                                results[fileToAnalyse]["builtIns"] = {"TypedArray": []};
                            } else if (!results[fileToAnalyse]["builtIns"].includes("TypedArray")) {
                                results[fileToAnalyse]["builtIns"]["TypedArray"] = [];
			    }
                        } else {
                            if (!results[fileToAnalyse].hasOwnProperty("builtIns")) {
                            ...
                            } else if (!results[fileToAnalyse]["builtIns"].hasOwnProperty(stmt.name)) {
                                results[fileToAnalyse]["builtIns"][stmt.name] = [];
			    }
                        }
            ...
            case "ArrayExpression":
                {
                    ...
                    } else if (!results[fileToAnalyse]["builtIns"].hasOwnProperty("Array")) {
                        results[fileToAnalyse]["builtIns"]["Array"] = [];
		    }
                    return stmt;
                }
            ...
            case "ObjectExpression":
                {
                    ...
                    } else if (!results[fileToAnalyse]["builtIns"].hasOwnProperty("Object")) {
                        results[fileToAnalyse]["builtIns"]["Object"] = [];
		    }
                    return stmt;
                }
...
var metadata = JSON.parse(readFileContent("metadata_test262.json"));
...
for (var i = 0; i < metadata.length; i++) {
    ...
    var path = fileToAnalyse.replace('test/', '');
    ...
    if (version.hasOwnProperty(fileToAnalyse)) {
        if (version[fileToAnalyse].substring(0, 2) === "es") {
            ...
            console.log("What happens?? (no `es` in version)")
        }
    }
